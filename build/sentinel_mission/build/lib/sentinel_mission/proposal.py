...
class DroneNode(Node):
    ...
    def _advance_init_step(self):
        ...
        """
        if self._init_busy:
            return  # re-entrancy guard (service calls spin the executor)
    ...
    def _step_request_mission(self):
        self.get_logger().info("Step 1: Requesting mission from Mission Node...")
        if not self._mission_cli.wait_for_service(timeout_sec=10.0):
            self.get_logger().error("Mission service not available")
            self._force_standby()
            return

        req = GetMission.Request()
        req.mission_file = ""  # use default
        ...
        self._next_step()  # advances to step 2
    ...
    def _step_upload_waypoints(self, elapsed):
        ...
        if not self._wp_push_cli.wait_for_service(timeout_sec=2.0):
        ...
        future = self._wp_push_cli.call_async(req)
        rclpy.spin_until_future_complete(self, future, timeout_sec=5.0)
        res = future.result()
    ...
    def _step_arm(self, elapsed):
        ...
        if self.fcu_state.armed:
            ...
            self._next_step()
        elif elapsed > self._arm_timeout:
            self.get_logger().error("Arming confirmation timeout")
            self._force_standby()
    ...
    def _step_takeoff(self, elapsed):
        ...
        if not self._takeoff_cli.wait_for_service(timeout_sec=2.0):
        ...
        future = self._takeoff_cli.call_async(takeoff_req)
        rclpy.spin_until_future_complete(self, future, timeout_sec=3.0)
        res = future.result()
        ...
        if self.local_pose.pose.position.z >= self._takeoff_alt - 0.5:
            ...
            self._next_step()
        elif elapsed > self._takeoff_timeout:
            self.get_logger().error(
